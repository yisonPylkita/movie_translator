...
//!
//! Builds a set of character names / proper nouns that should not be
//! translated, using capitalization heuristics over the English subtitle
//! dialogue. A candidate must appear at least 3 times to qualify.
...
static HONORIFIC_RE: LazyLock<Regex> = LazyLock::new(|| {
    Regex::new(r"(?:Sir|Lord|Lady|Princess|Prince|King|Queen|Master|Miss)\s+([A-Z][a-z]+)").unwrap()
});
...
/// Capitalize the first ASCII letter of `s`, lowercasing the rest.
...
/// Trim leading/trailing characters in the given set from `s`.
...
pub fn extract_proper_nouns_from_subtitles(dialogue_texts: &[String]) -> HashSet<String> {
    ...
    let mut common_false_positives: HashSet<String> = COMMON_FALSE_POSITIVES
        .iter()
        .map(|s| s.to_string())
        .collect();
    for h in HONORIFICS {
...
mod tests {
    ...
    // Expected sets are golden values: the reference output of
    // `extract_proper_nouns_from_subtitles` on these exact inputs.
