...
The shared `extract_burned_in_subtitles` only merges *exactly-equal*
consecutive frames, so OCR jitter on a static subtitle ("był" / "byl" /
"byt") becomes several near-duplicate lines, and transient garbage from the
OP/ED karaoke or on-screen signs leaks through. This module post-processes
the per-frame `OCRResult`s (text + timestamp) into clean lines:

  * fuzzy-merge consecutive frames whose text is *similar* into one timed
    block, keeping the best (most complete) variant;
  * drop non-dialogue noise by content (alpha ratio / min letters) and by
    persistence (a real line spans a minimum duration; single-frame OCR
    flicker does not) — this is anime-agnostic, so it handles OP/ED/sign
    junk wherever it appears without hardcoding per-series timings.

It deliberately lives in the PoC (not the shared OCR package) so the main
pipeline's OCR stays untouched.
"""
