...
class TestSubtitleProcessor:
    def test_extract_dialogue_lines_from_ass(self, create_ass_file):
        ...
        lines = SubtitleProcessor.extract_dialogue_lines(ass_file)
    ...
    def test_extract_dialogue_lines_from_srt(self, create_srt_file):
        ...
        lines = SubtitleProcessor.extract_dialogue_lines(srt_file)
    ...
    def test_raises_for_nonexistent_file(self):
        with pytest.raises(SubtitleProcessingError, match='not found'):
            SubtitleProcessor.extract_dialogue_lines(Path('/nonexistent/file.ass'))
    ...
    def test_raises_for_invalid_file(self, tmp_path):
        ...
        with pytest.raises(SubtitleProcessingError, match='Failed to parse'):
            SubtitleProcessor.extract_dialogue_lines(invalid_file)
...
""")
        ...
        lines = SubtitleProcessor.extract_dialogue_lines(no_dialogue_file)
        assert lines == []

    def test_create_english_subtitles(self, create_ass_file, tmp_path, sample_dialogue_lines):
        original_ass = create_ass_file()
        output_path = tmp_path / 'english_clean.ass'

        SubtitleProcessor.create_english_subtitles(original_ass, sample_dialogue_lines, output_path)

        assert output_path.exists()
        content = output_path.read_text()
        assert 'Hello, how are you?' in content

    def test_create_polish_subtitles_with_replacement(
    ...
    def test_create_polish_subtitles_without_replacement(
        self, create_ass_file, tmp_path, sample_translated_lines
    ):
        original_ass = create_ass_file()
        output_path = tmp_path / 'polish.ass'
