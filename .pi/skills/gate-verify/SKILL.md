---
name: gate-verify
description: Run the full gate chain (just check, just test, just py-test) to verify Rust and Python changes are green. Use after editing code in this repo before claiming done. Parses each gate's output and identifies exactly what failed.
---
...
4. If a gate needs the venv and it's missing (`ModuleNotFoundError`,
   ...
   `just deps` / the `pyo3-bridge-doctor` skill rather than calling it a
   code failure.
