...
function isTrackedFile(path: string): boolean {
  ...
  if (rel.startsWith("vendor/") || rel.startsWith("target/") || rel.startsWith(".venv/")) return false;
  return true;
}
...
async function formatOneFile(pi: ExtensionAPI, fp: string): Promise<string> {
  ...
  if (fp.endsWith(".rs")) {
    const r = await pi.exec("rustfmt", ["--edition", "2021", fp]);
    ...
    if (r.code !== 0) notes = `ruff check (unfixable):\n${r.stdout || r.stderr}`;
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("tool_result", async (event, _ctx) => {
    ...
    else if (input?.filePath && typeof input.filePath === "string") fp = input.filePath;
...
\`just check && just test && just py-test\` before finishing.`,
  ...
  pi.registerCommand("gate", {
    ...
    handler: async (_args, ctx) => {
      ...
      if (fail) {
        ...
        ctx.ui.notify("Fast gate: PASSED ✓\n\nRemember: run the full `just check && just test && just py-test` before committing.", "info");
      }
    },
  });
  ...
  pi.registerTool({
    ...
    description: "Run the fast format-check gates (rustfmt --check + ruff check) to verify the repo is clean before claiming done. Does NOT run clippy, tests, or pytest — those are the slow gates.",
    promptSnippet: "Run rustfmt --check and ruff check to verify formatting",
    promptGuidelines: ["Use check_fast_gate before claiming work is complete to verify formatting and lint pass."],
    ...
    async execute(_toolCallId, _params, _signal, _onUpdate, _ctx) {
      ...
      if (!dirty) {
        return {
          content: [{ type: "text", text: "✅ Working tree clean — nothing to gate." }],
          details: { clean: true },
        };
      }
      const fail = await runFastGates(pi, dirty);
